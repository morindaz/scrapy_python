...
class DbSpider(scrapy.Spider):
    ...
    def get_url(self,response):
        ...
        drug_type = response.xpath("//div[@class='sort_one']/a/h3/text()").extract()
        drug_type = [index.strip('\r\n').strip() for index in drug_type]

        for index, new_url in enumerate(urls_path):

            yield Request(url=new_url, callback=self.get_content, meta={"drug_type": drug_type[index]})
    ...
    def get_content(self, response):
        item = BaiduItem()
        item['type'] = response.meta['drug_type']
        ...
        try:
            ...
            yield Request(url=next_url, callback=self.get_content,meta={"drug_type": response.meta['drug_type']})
        except:
            pass
